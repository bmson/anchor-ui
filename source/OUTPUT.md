...
> **Style:** {{STYLE_NAME}}
> **Rhythm:** {{RHYTHM_TYPE}}
> **Whitespace:** {{WHITESPACE_LEVEL}}
> **Emphasis:** {{EMPHASIS_TYPE}}
> **Balance:** Symmetric
...
- **Core Constraint:** {{PRIMARY_CONSTRAINT}}
- **Rhythm & Grid:** {{RHYTHM_TYPE}}. Adjust padding to feel {{WHITESPACE_LEVEL}}.
- **Emphasis & Hierarchy:** {{EMPHASIS_TYPE}} emphasis. Communicate importance via {{HIERARCHY_TYPE}} methods.
...
:root {
  ...
  /* Geometry & Effects */
  --radius-sm: calc({{RADIUS_MAIN}} * 0.5);
  --radius-md: {{RADIUS_MAIN}};
  --radius-lg: calc({{RADIUS_MAIN}} * 1.5);
  --radius-full: 9999px;

  --border-weight: {{BORDER_WEIGHT}};
  --shadow-main: {{SHADOW_MAIN}};
  --blur-main: {{BLUR_MAIN}};

  /* Motion */
  --duration-fast: {{DUR_FAST}};
  --duration-base: {{DUR_BASE}};
  --duration-slow: {{DUR_SLOW}};
  --ease-standard: {{EASE_MAIN}};
...
When creating animations or transitions, animate ONLY composite properties: `transform`, `opacity`, and `filter`. NEVER animate `width`, `height`, `margin`, `padding`, or `box-shadow` if `{{MOTION_LEVEL}}` is Fluid or higher, as this triggers layout repaints.
...
- [ ] **Negative Constraint:** {{FORBIDDEN_ELEMENTS}}
...
- [ ] Are animations strictly adhering to the `{{MOTION_LEVEL}}` rules and durations?
